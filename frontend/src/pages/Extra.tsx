...
import {
  ...
  CalendarClock, CreditCard, RefreshCw, User, Lock,
  ChevronRight, Palette, Bell, Shield, Download,
} from 'lucide-react';
...
export function Analytics() {
  ...
  const savingsRate = summary?.income > 0
    ? Math.round(((summary.income - summary.expense) / summary.income) * 1000) / 10 : 0;
  const creditUtil  = summary?.creditCards?.limit > 0
    ? pct(summary.creditCards.totalDebt, summary.creditCards.limit) : 0;
  const goalProgress= summary?.milestones?.target > 0
    ? pct(summary.milestones.saved, summary.milestones.target) : 0;
    ...
    <div className="fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-5">
        ...
        <div className="pill-tabs">
          {PERIODS.map((p) => (
            <button key={p.value} className={`pill-tab ${period === p.value ? 'active' : ''}`}
        ...
        {[
          ...
          <div key={label} className="stat-card">
            <div className="flex items-center justify-between mb-2">
              <span className="muted text-xs uppercase tracking-wide font-semibold">{label}</span>
      ...
      <div className="space-y-4 mb-5">
        ...
        <div className="card p-4">
          ...
          {trendData.length === 0 ? (
            <div className="h-48 grid place-items-center muted text-sm">No data available.</div>
          ) : (
            <div style={{ height: 200 }}>
              <ResponsiveContainer>
                <BarChart data={trendData} barGap={3} margin={{ left: -20, right: 4, top: 4, bottom: 0 }}>
                  ...
                  <Tooltip contentStyle={{ background: '#0d0f16', border: '1px solid #252836', borderRadius: 10, fontSize: 12 }}
        ...
        <div className="card p-4">
          ...
          {catData.length === 0 ? (
            <div className="h-48 grid place-items-center muted text-sm text-center">No expenses for this period.</div>
          ) : (
            <>
              <div style={{ height: 200 }}>
                <ResponsiveContainer>
                ...
                {catData.slice(0, 8).map((c, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center gap-3">
                      <div className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ background: c.color || COLORS[i % COLORS.length] }} />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-xs font-medium truncate">{c.name}</span>
                          <span className="text-xs font-semibold flex-shrink-0 ml-2">{money(c.value)}</span>
                        </div>
                        <div className="progress-track" style={{ height: 4 }}>
                          <div className="progress-fill" style={{ width: `${share}%`, background: c.color || COLORS[i % COLORS.length] }} />
                        </div>
                      </div>
                      <span className="text-xs muted flex-shrink-0 w-8 text-right">{share}%</span>
                    </div>
                  );
                })}
              </div>
...
export function Upcoming() {
  ...
  return (
    <div className="fade-in">
      ...
      <p className="muted mb-5 text-sm">Bills, subscriptions and card due dates.</p>

      {/* Credit card due dates */}
      {cards.length > 0 && (
        <div className="mb-5">
          <h2 className="text-xs font-bold muted uppercase tracking-wider mb-3">Credit Card Payments</h2>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs muted mt-0.5">
                          Due {dueDay}{['st','nd','rd'][((dueDay+90)%100-10)%10-1]||'th'} every month
                        </div>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-base font-bold" style={{ color: '#f43f5e' }}>
                        {money(card.outstandingBalance || 0)}
                      </div>
                      <div className="text-xs mt-0.5" style={{ color: isUrgent ? '#f59e0b' : '#636878' }}>
      ...
      {recurring.length > 0 ? (
        <div>
          <h2 className="text-xs font-bold muted uppercase tracking-wider mb-3">Recurring Transactions</h2>
          <div className="space-y-3">
            {recurring.map((r) => {
              ...
              return (
                <div key={r._id} className="card p-4">
                  <div className="flex items-center gap-3">
                    <CategoryIcon icon={cat?.icon || 'RefreshCw'} color={cat?.color || '#8b5cf6'} size={16} bgSize={42} />
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-semibold truncate">{r.description}</div>
                      <div className="text-xs muted mt-0.5">
                        {r.frequency} · {cat?.name}
                        {r.nextDueDate && ` · ${new Date(r.nextDueDate).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}`}
                      </div>
                    </div>
                    <div className="text-base font-bold flex-shrink-0" style={{ color: isIncome ? '#22c55e' : '#f43f5e' }}>
        ...
        <div className="card p-10 text-center">
          <CalendarClock size={40} className="mx-auto mb-4 muted" />
          <div className="font-semibold mb-1">No upcoming payments</div>
          <div className="text-sm muted">Add recurring transactions or set card due dates.</div>
        </div>
      )}
    </div>
  );
...
export function Settings() {
  ...
  const changePassword = async (e: React.FormEvent) => {
    ...
    setPwdSaving(true); setMsg(''); setErr('');
    try {
  ...
  return (
    <div className="fade-in max-w-2xl">
      ...
      {/* Feedback banners */}
      {msg && (
        <div className="mb-4 px-4 py-3 rounded-2xl text-sm" style={{ background: 'rgba(34,197,94,0.1)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.2)' }}>
          ✓ {msg}
        </div>
      )}
      {err && (
        <div className="mb-4 px-4 py-3 rounded-2xl text-sm" style={{ background: 'rgba(244,63,94,0.1)', color: '#f43f5e', border: '1px solid rgba(244,63,94,0.2)' }}>
          ⚠ {err}
        </div>
      )}
      ...
      <div className="card p-5 mb-4">
        <div className="flex items-center gap-4 mb-5">
          <div
            className="w-16 h-16 rounded-full grid place-items-center text-2xl font-bold flex-shrink-0"
      ...
      <div className="card p-5 mb-4">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-9 h-9 rounded-xl grid place-items-center" style={{ background: 'rgba(139,92,246,0.15)' }}>
        ...
        <form onSubmit={changePassword} className="space-y-4">
          ...
          <div>
            ...
            <input className="input" type="password" value={pwdForm.confirmPwd}
              onChange={(e) => setPwdForm({ ...pwdForm, confirmPwd: e.target.value })} required />
          </div>
          <button className="btn btn-primary w-full" type="submit" disabled={pwdSaving}>
      ...
      <div className="card p-5">
        <div className="flex items-center gap-3 mb-3">
          <div className="w-9 h-9 rounded-xl grid place-items-center" style={{ background: 'rgba(34,197,94,0.15)' }}>
            <Download size={18} style={{ color: '#22c55e' }} />
          </div>
          <h2 className="font-semibold text-base">Default Categories</h2>
        </div>
        <p className="muted text-sm mb-4">
          If your default categories are missing, restore them below. This will not create duplicates.
        </p>
        <button className="btn btn-secondary w-full" onClick={seedCategories} disabled={seeding}>
